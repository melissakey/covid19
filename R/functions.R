...
SIRX <- function(time, state, parameters) {
  ...
  with(par, {
    ...
    dR <- + kappa * I + kappa0 * I
    dX <- + kappa0 * S
    # dR <- beta * I          # This is likely not necessary because beta is fixed, not estimated.  That said, my estimate of R stays at 0.
    list(c(dS, dI, dR, dX))
  })
}
...
RSS <- function(parameters, data, init, fun, N) {
  if(fun == 'SIR') {                      # SIR model (directly parallels the SIRX code, but can be ignored.
    ...
    init <- data %>%
      ...
      set_names(c("X", "I", "S", "R"))     # fix names of the 4 elements since these did not ocome through correctly.
    ...
    res <- parms["N"] * fit$X - data$Infected  # calculate the residuals
  }
        ...
        function(data, N, fun = fun, R0 = 6.2, beta = 1/8, I0_factor = 10, method = c("confirmed", "shutdown", "quarantine")) {
          if(fun == 'SIR') {
            ...
            lower <- switch(method,
              ...
              'quarantine' = c(beta = beta, alpha = R0 * beta, kappa = 0, kappa0 = 0, I0_factor = 0.001, N = 10)
            )
