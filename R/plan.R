plan <- drake_plan(
  ...
  SIRX_predictions = predict_infections(data, new_times = 1:70, fun = "SIRX"),
  
  # SIR_prediction_plot = pmap(list(data$Province, data$data, SIR_predictions$fit), plot_predictions),
  SIRX_prediction_plot = pmap(list(data$Province, data$data, SIRX_predictions$fit), plot_predictions),
